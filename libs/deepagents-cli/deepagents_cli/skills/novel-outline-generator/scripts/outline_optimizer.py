...
class OutlineOptimizer:
    ...
    def optimize_chapter(self, chapter_num, chapter_data):
        ...
        # 优化章节概述
        optimized["overview"] = self._optimize_overview(chapter_num, chapter_data["overview"])
    ...
    def _optimize_plot_points(self, chapter_num, plot_points):
        ...
        for match in point_pattern.finditer(plot_points):
            ...
            points.append({"title": point_title, "content": point_content})
    ...
    def _make_specific(self, content, chapter_num):
        ...
        if "获得军功" in content:
            ...
            return content.replace(
                "获得军功", '蒙卡对李寒刮目相看："没想到你小子还有这本事。这次记你一等功！"'
            )
    ...
    def _generate_output(self):
        ...
        for chapter_num, chapter_data in self.chapters.items():
            if (
                chapter_num
                in range(
                    int(self.chapter_range.split("-")[0]), int(self.chapter_range.split("-")[1]) + 1
                )
                if self.chapter_range
                else range(1, len(self.chapters) + 1)
            ):
                # 查找并替换章节内容
                chapter_pattern = re.compile(
                    rf"(### 第{chapter_num}章：《.+?》\n\n\*\*章节概述\*\*:.+?\n\n\*\*情节点\*\*:)(.+?)(?=\n\n\*\*情感变化\*\*:)",
                ...
                output_content = chapter_pattern.sub(
                    rf"\1{chapter_data['plot_points']}", output_content
                )
...
def main():
    ...
    optimizer = OutlineOptimizer(
        input_file=args.input, output_file=args.output, chapter_range=args.chapters
    )
